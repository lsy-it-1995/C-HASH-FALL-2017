...
template<class RecordType>
size_t open_hash<RecordType>::collisions()
{
    return collision;
}

template <class RecordType>
void open_hash<RecordType>::insert(const record &entry)
{
...
template <class RecordType>
bool open_hash<RecordType>::remove(int key)
{
    bool found = false;
    size_t index;
    assert(key>=0);
...
template<class RecordType>
bool open_hash<RecordType>::find(int key, bool &found, record &result) const
{
    size_t index;
    find_index(key,found,index);
    if(found)
...
template<class RecordType>
size_t open_hash<RecordType>::hash(int key) const
{
    return key%CAPACITY;
}

template<class RecordType>
size_t open_hash<RecordType>::next_index(size_t index) const
{
    bool debug = false;
...
template<class RecordType>
void open_hash<RecordType>::find_index(int key, bool &found, size_t &index) const
{
    size_t count;
    count = 0;
    index = hash(key);
...
template<class RecordType>
bool open_hash<RecordType>:: never_used(size_t index) const
{
    return (data[index]._key==NEVER_USED);
}

template<class RecordType>
bool open_hash<RecordType>::is_vacant(size_t index) const
{
    bool debug = false;
